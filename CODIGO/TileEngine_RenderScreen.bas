...
Sub RenderScreen(ByVal center_x As Integer, _
    ...
    StartX = CInt(ViewportOffsetX) + PixelOffsetX - MinX * TilePixelWidth + gameplay_render_offset.x
    StartY = CInt(ViewportOffsetY) + PixelOffsetY - MinY * TilePixelHeight + gameplay_render_offset.y
    ' Screen start with tiles buffered (for layer 2, chars, big objects, etc.)
    StartBufferedX = StartX + TileBufferPixelOffsetX + MinX * TilePixelWidth  ' Derivado de StartX para alinear capas
    StartBufferedY = StartY - TilePixelHeight + MinY * TilePixelHeight  ' Derivado de StartY para alinear capas
    ...
    If MinX < XMinMapSize Then
        StartBufferedX = CInt(ViewportOffsetX) + PixelOffsetX - MinX * TilePixelWidth
        MaxX = MaxX - MinX
    ...
    If MinY < YMinMapSize Then
        StartBufferedY = CInt(ViewportOffsetY) + PixelOffsetY - MinY * TilePixelHeight
        MaxY = MaxY - MinY
    ...
    ' === DEBUG CENTRO ===
    Call Engine_Draw_Box(666 - 30, 464, 60, 2, RGBA_From_Comp(255, 0, 0, 200))
    Call Engine_Draw_Box(666, 464 - 30, 2, 60, RGBA_From_Comp(255, 0, 0, 200))
    Call RenderText("Pos: " & UserPos.x & "," & UserPos.y, 670, 468, COLOR_WHITE)
    ' === FIN DEBUG ===
